...
## [v3.5.0] - 2026-09-10
### 修复与优化（MINOR：NVIDIA DRS 写入根因修复 + 异类策略缺失自动注入 + Toast 位置优化）

用户实测反馈三项问题，全部根因定位（真机诊断实证）后修复：

- **修复：NVIDIA 电源管理"写入后验证不一致"（期望 1 读回 0）——写入从未生效**：
  - 根因（DRS 值 union 重叠语义误用）：DWORD 类型设置的真实值位于值 union **起始 4 字节**
    （u32CurrentValue）；旧实现把该处当 `value_length` 恒写 4，再把值写入 union 偏移
    4..8 —— u32 值位被写成常量 4（无效值），驱动静默回退默认，写入从未真实生效；
    诊断探针实证：本机存储值恰为坏写遗留的 4；
  - 修复：DWORD 读写一律走 union u32 值位（`NvDrsUnion.u32_value`）；结构总大小 12320 不变；
  - 补上 **`NvAPI_Initialize`**（NVAPI 契约强制的首调用，上游即缺失）；
  - 验证会话分离：写入（Get→Set→Save）与验证（新会话 LoadSettings 读盘）独立，
    杜绝任何脏缓存误判；
  - 读取降级：存储值无效时按驱动语义报告默认自适应（UI 可正常选择修复，不再整块报错）；
  - 真机闭环：变值写最高性能优先 → 同会话回读 1 → 新会话读盘 1 → 恢复自适应，全通过；
- **新增：核心线程调度策略缺失自动注入**：
  - `get_hetero_policies` 新增 `thread_present/short_present` 方案级存在性检测；
  - 真机机制实证：`PowerWrite*ValueIndex` 对方案下不存在的设置项键**自动创建并落值**
    （电源服务链路，与 powercfg 同源）——注入内建于写入路径；
  - 重要教训（探针实证）：PowerSchemes 子键受电源服务 ACL 保护，管理员直写注册表也被
    拒绝——放弃注册表直写注入路线；
  - UI：异类策略卡不再因"不支持"硬阻断——设置项缺失时显示警告提示
    （"首次配置时将自动注入当前电源计划"），四组六档始终可配置，保证配置成功；
- **优化：Toast 泡泡栈移至中间显示区右上角**——原钉在窗口级顶层（覆盖右栏日志流），
  现挂载到 Main 列页面区容器内（absolute 右上角，不遮侧栏/顶栏/右栏）。

## [v3.4.0] - 2026-09-10
### 新功能（MINOR：系统设置页全链路审计与完整迁移，ADR-0001）

对齐上游 `youridol/sysenv-console-manager` `/settings` 页全量能力（Tauri+React → 纯 Rust+GPUI），
真机（Administrator）读写链路逐项验证闭环。

- **新增：关闭高精度计时器开关（secm-core::hpt）**：
  - bcdedit 三条目真实读写（`useplatformclock/useplatformtick/disabledynamictick`），
    `CREATE_NO_WINDOW` 防控制台闪烁；布尔解析兼容本地化词表（是/否/Yes/No/true/false）；
  - 设置页「系统类」分组独立开关（管理员徽标 + 需重启生效说明）；
  - 真机完整往返验证：关闭 → 三条齐备判定 enabled=true → 恢复默认 → enabled=false。
- **新增：NVIDIA 显卡电源管理（secm-core::nvidia_drs，NVAPI DRS）**：
  - `LoadLibraryW(nvapi64.dll)` + `nvapi_QueryInterface` 序数解析 8 个 DRS 接口；
    `NvDRS_SETTING_V1`（12320 字节 pack(4)）按名解析「Power management mode」settingId；
  - 值域 0=自适应 / 1=最高性能优先 / 5=最佳功率（NVIDIA App featureEnum 权威）；
  - 优雅降级：无 NVIDIA 卡/驱动未装 → 明确错误态；全局 profile 无设置项（-160）→
    读返回驱动默认自适应、写构造新设置项；写后 SaveSettings + 重读校验防驱动静默拒绝；
  - 真机验证：RTX 环境 GetSetting/SetSetting/SaveSettings/校验读全链路通过；
  - UI 三档选择组（当前档高亮、写操作互斥禁点、失败回读真实值）。
- **新增：电源计划删除 + 二次确认**（上游 AlertDialog 等价）：
  - `ui::page::confirm_modal` 模态弹层原语（gpui::deferred 置顶 + occlude 遮罩），
    设置页（删除计划）与服务页（停止服务）共用；
  - 行内删除钮仅非激活计划展示；`PowerDeleteScheme` 真机验证（含激活计划 API 拒绝兜底）。
- **新增：电源计划滑动开关激活**（行内 Switch：激活置亮禁点 / 其余点击切换）+
  Win11 overlay 计划过滤（GUID×2 + 名称含 overlay）+ GUID→中文名映射 +
  FriendlyName 间接串（`@dll,-id,后缀`）解析（真机：高性能/节能/平衡/卓越性能正确显示）。
- **新增：卓越性能计划首载自动导入 + 自动激活**（对齐上游 useEffect 行为；
  失败仅记日志不阻塞页面；手动导入按钮保留）。
- **新增：异类线程调度策略 AC/DC 独立六档 UI**（线程/短运行 × AC/DC 四组六档；
  `set_hetero_policy_scoped` 作用域写入保留 AC 回滚；非混合架构降级说明；
  真机：supported=true 全链路写回验证）。
- **修复：服务启动类型全量"未知"根因修复**——`enum_services` 占位值无调用方补全
  （上游 8 线程并行补全链路缺失），`list_all_services` 补 8 线程
  `QueryServiceConfigW` 并行补全（真机 326 服务：自动/手动/已禁用正确显示）；
- **新增：服务页启动类型三档按钮组（自动/手动/禁用）+ 停止服务二次确认弹层**
  （原 ToggleAuto/Manual/Disable dead_code 全部接线销核）。
- **UI**：开关行补管理员徽标 + 静态描述 + 真实状态消息三行结构；
  设置页重排为 优化开关｜电源计划（两列）+ 核心策略 + NVIDIA（全宽）四卡信息层级；
  全部写操作全局互斥（BusyOp 定位对应控件置灰禁点），Toast + 横幅 + 日志流统一反馈。

## [v3.3.4] - 2026-09-09
### 修复（PATCH：进程管理表格 PID/进程名/内存 三列居中显示）

- **调整：进程管理 PID / 进程名 / 内存 三列字体水平居中（用户指令）**：
  - 列规格重构：`ColWidth::Num`（定宽右对齐）→ `Mid`（定宽居中）+ 新增 `FlexMid`
    （弹性居中）；`table_num_cell` → `table_mid_cell`（居中单元格原语沉淀在
    `ui::page` 全局层）；
  - 表头与数据行同规格：PID（70 定宽居中）/ 进程名（弹性居中，超长仍单行省略）/
    内存（90 定宽居中）；优先级按钮组保持左对齐不变；
  - 居中对齐原语可复用，后续表格按语义选 Flex/Px/FlexMid/Mid。

## [v3.3.3] - 2026-09-09
### 修复（PATCH：数据表行根因级修复 — uniform_list 行宽收缩导致全列拥挤错位）

- **修复：进程管理表格行不占满容器宽 → 表头与行全列错位（用户复现，截图实证）**：
  - 根因：`uniform_list` 以 `item.layout_as_root()` 装载行元素 —— 行是**布局根**，
    无父容器 cross-axis stretch，taffy 对 `width:auto` 的布局根按内容收缩
    （shrink-to-fit）→ 行仅 ~570px 内容宽挤在左侧，与全宽表头（card flex_col
    内 stretch 全宽）逐列错位：表头"PID/进程名/内存/优先级"与行内单元格完全对不上；
  - 修复：`ui::page::table_row()` 强制 `.w_full()`（行恒占满容器宽，flex_1 名称列
    正确伸展、数字列右缘与表头严格成列）；`table_head()` 同步补齐 `.w_full()`
    （防御一致）；
  - 回归验证：PrintWindow 实截对比（修前：行内容止于 ~570px、右侧大片空白；
    修后：行全宽、四列表头与数据逐列对齐）；
  - 该修复惠及全部 uniform_list 表格场景（当前为进程管理 Top 200），
    普通滚动容器内的表格（服务管理/硬件检测）行为不变（原本即 stretch 全宽）。

## [v3.3.2] - 2026-09-09
### 修复（PATCH：进程管理表格列对齐优化）

- **优化：进程管理进程条目列对齐（用户指令）**：
  - PID / 内存为数字列 → 统一**右对齐**（新增 `ColWidth::Num` 表头规格 +
    `table_num_cell` 数据行单元格，同一列同一宽度 → 表头与各行数值右缘严格成列，
    位数不齐（如 "4" 与 "12345"、"512 MB" 与 "1.2 GB"）不再参差）；
  - 进程名（文本列）保持左对齐、优先级按钮组保持左对齐 —— 文本左对齐、数值右对齐的
    表格对齐规范；对齐原语沉淀在 `ui::page` 全局层，后续表格可复用。

## [v3.3.1] - 2026-09-09
### 修复（PATCH：清理优化页排版调整）

- **调整：清理优化页上下两行全宽排版（用户指令）**：
  - 「缓存清理」与「进程管理」由左右两列并排改为**上下两行、各自全宽**；
  - 纵向节奏由卡片自带 `.mb(PAGE_GAP)` 承担；进程管理表格随全宽自适应进程名，
    名称列可读空间显著增加；功能逻辑零变更。

## [v3.3.0] - 2026-09-09
### 新增 + 全链路 UI 统一（MINOR：全局泡泡提示系统 + 输入框视觉规范 + 两列响应式卡片网格 + 按钮文案统一）

- **新增：全局泡泡提示系统 `secm-app::ui::toast`（Toast）**：
  - 全局右上角覆盖层（钉于顶栏下方，绘制在壳最上层；覆盖层本体无事件监听，空白处点击穿透）；
  - 四类语义反馈：成功（绿）/ 警告（黄）/ 错误（红）/ 提醒（中性），语义色/描边/图标全局统一；
  - 默认 **1.6s 自动消失**（1–2s 区间内，按最近到期剩余时长续约调度），支持点击 × 手动关闭；
  - 入场动画：淡入 + 轻微下滑（gpui Animation + ease_out_quint，170ms）；
  - 同屏上限 4 条（超出丢最旧防刷屏）；单条宽 ≤420px 超长省略；
  - 架构：`ToastHost` 队列实体（壳持有）+ `ToastGlobal`（GPUI Global 弱句柄）——任意页面
    Context 一行推送（`toast::success/warning/error/info`），渲染统一由 `render_stack` 装配，
    页面零重复实现；壳订阅队列变更驱动重绘；新增 3 枚语义图标（toast-success/warning/error.svg）；
  - 接入点（必要操作结果随屏可见）：清理优化（清理/DNS/优先级完成摘要）、服务管理（启停/启动类型）、
    系统设置（开关/电源计划/调度策略/卓越导入）、网络配置（应用结果/MAC/前置校验）、
    网络诊断（站点增删改与持久化失败）、环境检测（预设套用）、AI 环境（安装/升级/卸载）、硬件检测（SMART 读取失败）。
- **修复：输入框全链路视觉统一（`ui::text_input` 重构）**：
  - **修复输入文字继承黑色不可读**：文本色显式取 Palette（暗色主题高对比白字），
    placeholder 取 text_dim，光标取 accent，选区取 accent 软色（明暗随壳主题联动）；
  - 统一外壳：圆角 8 / 1px 描边 border / surface_muted 底 / 高 32（与按钮对齐）；
    焦点态 accent 描边 + surface 底；新增禁用态（50% 透明度 + 全部输入交互门禁拒绝，
    netsh 应用期间网络配置输入框自动锁定）；
  - 视觉只此一处定义，全部 20+ 输入框实例（net_config 7 / network 12 / cleanup / services）自动生效。
- **新增：两列响应式卡片网格（`ui::page::card_grid_row/grid_cell`）**：
  - 容器级自适应（flex_wrap + 460px 基准宽 + flex_grow 均分）：主区足够宽两卡并排各占一半，
    过窄（含右栏日志面板挤压主区的场景）自动换行为单列全宽 —— 无需窗口断点、无视口误判；
  - 接入页面：系统设置（开关|电源计划）、网络配置（当前配置|IPv4、IPv6|MAC 两行）、
    清理优化（缓存清理|进程管理，替换原 900px 视口断点）、环境检测（系统信息|AI 工具、DX|VC++）、
    AI 环境（npm|工具、MCP|扩展）、关于（产品|信息）；仪表盘/网络诊断原有行列保持。
- **整理：按钮全链路统一**：
  - 防挤压规范：button/button_sm 统一 `whitespace_nowrap + flex_shrink_0`（永不收缩、永不换行）；
  - 文案统一简化（不影响语义）：切换为 DHCP（自动获取）→切换 DHCP、应用静态 IPv4 + DNS→应用 IPv4/DNS、
    应用静态 IPv6→应用 IPv6、刷新配置→刷新、一键清理全部着色器缓存→全清着色器、
    NVIDIA/AMD/Steam 着色器缓存→清 NVIDIA/AMD/Steam 缓存、刷新 DNS 缓存→刷新 DNS、
    刷新进程列表→刷新进程、导入卓越性能计划→导入卓越计划、刷新磁盘列表→刷新、刷新磁盘与 SMART→刷新磁盘。
- **修复：进程条目/表格单元格自适应**：
  - 进程管理行名称列改单行省略（whitespace_nowrap + truncate），杜绝固定行高 44px 下的
    异常换行与溢出；服务管理显示名/服务名列同规范；窄容器自动省略不破版。
- **工程约束**：全部新组件落在 `ui::` 全局层（toast/输入外壳/网格），色板只取
  `pi_clone::theme::Palette`（新增 `ThemeGlobal` 全局外观 + `current_palette`），功能逻辑零变更。

## [v3.2.1] - 2026-09-08
### 修复 + 整理（PATCH：托盘图标失效根因修复 + 壳/清理优化页 UI 调整）

- **修复：托盘图标失效（根因级）**：
  - 根因：`tray-icon 0.24` Windows 平台 `impl Drop for TrayIcon` 即执行
    `remove_tray_icon → Shell_NotifyIconW(NIM_DELETE)`（源码 mod.rs:297 实证）；
    历史实现 `.build().is_err()` 使返回的 `TrayIcon` 为临时值被立即 Drop——
    托盘图标创建后即刻被系统移除；
  - 修复：`TrayIcon` 持有至托盘线程消息循环结束（线程退出时 Drop 才移除，
    对齐应用退出路径）；创建成功写入 Info 日志（`托盘图标已创建`，可观测验证）。
- **修复：左侧边栏头部品牌图标视觉**：
  - 原 logo 为无内容 accent 空色块（部分主题下视觉近白），改为品牌 Terminal
    图标（accent 色 + 选中底色衬底）；标题栏文本按用户指令移除
    （窗口名仍保留于任务栏/Alt-Tab）。
- **整理：清理优化页**：
  - 执行结果追溯并入右侧日志流：拆除页内「执行结果追溯」卡（历史/明细双轨重复），
    全部清理与快捷操作的执行触发、完成摘要、明细逐行（失败/重启删除 Warn 呈现）
    流式记录至右侧日志流面板，右侧栏为唯一追溯出口；
  - 快捷操作卡拆除：「刷新 DNS 缓存」移至「清理临时文件」右侧（系统临时分组）；
    「刷新进程列表」与进程搜索输入框并入进程管理卡容器（表头上方工具栏，搜索框弹性占满）；
  - 进程管理标题右侧小字描述改为「进程的CPU优先级」（保留计数信息）。
